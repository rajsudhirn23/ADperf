...
test('test user 1', async ({ page }) => {
  ...
  // const continuePaymentBtn = page.getByRole('button', { name: 'Continue to payment right-' });
  // const isContinuePaymentVisible = await continuePaymentBtn.waitFor({ state: 'visible', timeout: 5000 }).then(() => true).catch(() => false);
  // // You can use this boolean for assertions or logging
  // console.log('Continue to payment button visible:', isContinuePaymentVisible);
  // await page.waitForTimeout(1000);
...
//   await page.getByRole('textbox', { name: 'Fault date - MM / DD / YYYY' }).fill('10/10/2025');
//   await page.getByRole('button', { name: 'Continue right-chevron' }).click();
...
//   await page.locator('form').filter({ hasText: 'Tue214slots available' }).getByLabel('show slots').click();
